...
public class RowSeatAdapter extends RecyclerView.Adapter<RowSeatAdapter.RowSeatViewHolder> {
    ...
    public void onBindViewHolder(@NonNull RowSeatViewHolder holder, int position){
        ...
        updateStateSeat(holder.button_seat_Ai, item.getStateSeatA_i(), position == selectedPosition && item.getSelectedSeatType() == SeatType.A);
        updateStateSeat(holder.button_seat_Bi, item.getStateSeatB_i(), position == selectedPosition && item.getSelectedSeatType() == SeatType.B);
        updateStateSeat(holder.button_seat_Ci, item.getStateSeatC_i(), position == selectedPosition && item.getSelectedSeatType() == SeatType.C);
        updateStateSeat(holder.button_seat_Di, item.getStateSeatD_i(), position == selectedPosition && item.getSelectedSeatType() == SeatType.D);
    }
    ...
    private class SeatClickListener implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            RowSeatItem currentItem = listRowSeat.get(position);
            if (selectedPosition == position && currentItem.getSelectedSeatType() == seatType) {
                selectedPosition = RecyclerView.NO_POSITION;
                currentItem.setSelectedSeatType(null);
            } else {
                selectedPosition = position;
                currentItem.setSelectedSeatType(seatType);
            }
